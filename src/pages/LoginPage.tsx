...
export function LoginPage({ navigate }: { navigate: (to: string) => void }) {
  ...
  const { signIn, signInWithGoogle, signUp, sendResetEmail, updatePassword, isRecovery, clearRecovery } = useAuth();
              ...
              <form onSubmit={handleSignIn} className="space-y-4">
                ...
                <div className="relative py-1">
                  ...
                  <div className="relative flex justify-center text-xs">
                    <span className="px-3 bg-white dark:bg-slate-900 text-slate-400">{lang === 'te' ? 'లేదా' : lang === 'hi' ? 'या' : 'or'}</span>
                  </div>
                </div>

                {/* Google sign-in */}
                <button type="button" onClick={handleGoogleSignIn} disabled={loading} className="btn w-full !bg-white dark:!bg-slate-800 !text-slate-700 dark:!text-slate-200 border border-slate-300 dark:border-slate-600 hover:!bg-slate-50 dark:hover:!bg-slate-700 text-sm font-semibold">
                  <GoogleIcon className="w-5 h-5" />
                  {lang === 'te' ? 'Googleతో కొనసాగండి' : lang === 'hi' ? 'Google के साथ जारी रखें' : 'Continue with Google'}
                </button>
              </form>
            )}
